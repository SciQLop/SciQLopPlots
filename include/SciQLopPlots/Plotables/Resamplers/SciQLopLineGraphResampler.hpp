...
struct QCPGraphDataVectorIterator
{
    ...
    QCPGraphDataVectorIterator(QVector<QCPGraphData>::const_iterator it)
            : _it(it) {}

    QCPGraphDataVectorIterator& operator++() { ++_it; return *this; }
    QCPGraphDataVectorIterator operator++(int) { auto tmp = *this; ++_it; return tmp; }
    QCPGraphDataVectorIterator& operator--() { --_it; return *this; }
    QCPGraphDataVectorIterator operator--(int) { auto tmp = *this; --_it; return tmp; }
    QCPGraphDataVectorIterator& operator+=(difference_type n) { _it += n; return *this; }
...
struct ResamplingContextLevelView
{
    ...
    {
        return std::distance(keys_begin, keys_end);
    }
};
                                             ...
                                             std::size_t dest_size)
{
    ...
    QVector<QCPGraphData> data(dest_size);
    {
        ...
        data[1].value = nan;
        for (auto x_it = x_begin, y_it = y_begin, idx=0UL; idx<source_size; x_it++, y_it++, idx++)
        {
            auto x = *x_it;
            auto y = *y_it;
...
struct ResamplingContextLevel
{
    ...
    inline void update(const XYView& view, std::size_t column_index)
    {
        ...
        data=resample(view.x_row_iterator(0), view.y_row_iterator(0, column_index), std::size(view), 1'000'000);
    ...
    inline const auto keys_end() const
    {
        return QCPGraphDataVectorIterator<false>(data->cend());
    }
    ...
    inline const auto values_end() const
    {
        return QCPGraphDataVectorIterator<true>(data->cend());
    }

    inline ResamplingContextLevelView view(const double xmin, const double xmax) const
    {
        if (!contains(xmin, xmax))
            return ResamplingContextLevelView { keys_begin(), keys_end(), values_begin(), values_end() };

        auto begin_it = std::lower_bound(data->cbegin(), data->cend(), xmin,
                                         [](const QCPGraphData& data, double x) { return data.key < x; });
        auto end_it = std::upper_bound(data->cbegin(), data->cend(), xmax,
                                       [](double x, const QCPGraphData& data) { return x < data.key; });

        return ResamplingContextLevelView {
            QCPGraphDataVectorIterator<false>(begin_it),
                                             ...
                                             std::size_t dest_size, ResamplingContextLevel& context)
{
    ...
    if(view_size >= 10'000'000)
    {
        ...
        return resample(context_view.keys_begin, context_view.values_begin,
                        context_view.size(),
                        dest_size);

    }
    return resample(view.x_row_iterator(0), view.y_row_iterator(0, column_index),
                    view_size,
                    dest_size);
}
